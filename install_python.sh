#!/usr/bin/env bash
# bootstrap-python-multiversion.sh
#
# Goals:
# - Build and install multiple CPython versions under BASE_DIR/opt (altinstall)
# - Provide shims + entrypoints under BASE_DIR/bin and BASE_DIR/shims
# - Install/Update Miniforge to BASE_DIR/opt/conda with isolated caches under BASE_DIR/CACHE
# - Idempotent, non-interactive, avoid editing user's shell init unexpectedly

set -euo pipefail
umask 022

# ---- hard safety: must be run as a normal user (can sudo when needed) ----
if [[ "${EUID:-$(id -u)}" -eq 0 ]]; then
  echo "FATAL: do not run as root; run as a normal user (with sudo available if needed)." >&2
  exit 1
fi
if [[ -n "${SUDO_USER:-}" ]]; then
  echo "FATAL: do not run under sudo; run as a normal user." >&2
  exit 1
fi

# ---- error trap for actionable failures ----
on_err() {
  local ec="$?"
  echo "FATAL: failed at line ${BASH_LINENO[0]}: ${BASH_COMMAND}" >&2
  exit "${ec}"
}
trap on_err ERR
...
Notes:
- BASE_DIR is any directory. Script will create subdirs under it.
- --reuse allows running again when BASE_DIR already exists.
- --skip-apt assumes build dependencies are already installed.
- --skip-gpg skips OpenPGP verification (NOT recommended).
- --no-bashrc will NOT append BASE_DIR/bin PATH block to user's ~/.bashrc.
EOF
  exit 2
}
...
# ===== pinned versions =====
# Python 3.13.11 release page: 2025-12-05
PY313="3.13.11"
PY311="3.11.13"
PY310="3.10.18"
PY38="3.8.19"

# build newest first (helps detect toolchain problems earlier)
PY_VERSIONS=("${PY313}" "${PY311}" "${PY310}" "${PY38}")

MINIFORGE_VER="25.9.1-0"
MINIFORGE_FN="Miniforge3-${MINIFORGE_VER}-Linux-x86_64.sh"

# OpenPGP key IDs per python.org OpenPGP Verification page:
# - Thomas Wouters (3.12.x/3.13.x source): A821E680E5FA6305
# - Pablo Galindo Salgado (3.10.x/3.11.x source): 64E628F8D684696D
# - Łukasz Langa (3.8.x/3.9.x source): B26995E310250568
PY_PGP_KEYS=("A821E680E5FA6305" "64E628F8D684696D" "B26995E310250568")

# Prefer stable HTTPS public-key sources linked from python.org metadata page.
# (Fallback to keyservers if these URLs fail.)
declare -A PGP_KEY_URLS
PGP_KEY_URLS["A821E680E5FA6305"]="https://github.com/Yhg1s.gpg"
PGP_KEY_URLS["64E628F8D684696D"]="https://keybase.io/pablogsal/pgp_keys.asc?fingerprint=a035c8c19219ba821ecea86b64e628f8d684696d"
PGP_KEY_URLS["B26995E310250568"]="https://keybase.io/ambv/pgp_keys.asc?fingerprint=e3ff2839c048b25c084debe9b26995e310250568"
...
install_deps() {
  ...
  as_root apt-get install -y \
    ...
    libexpat1-dev libmpdec-dev libtirpc-dev \
    acl
}

preflight() {
  log "Preflight"
  for c in bash id dirname date; do have "$c" || die "Missing command: $c"; done
  have sudo || die "sudo missing (required to install deps / altinstall / chown)"
}
...
import_python_keys() {
  ...
  local tmp="${BASE_DIR}/CACHE/tmp/pgpkey_${RANDOM}_$$.asc"
  ...
  for kid in "${PY_PGP_KEYS[@]}"; do
    ...
    local got=0
    local url="${PGP_KEY_URLS[${kid}]:-}"
    if [[ -n "${url}" ]]; then
      log "Fetching Python OpenPGP key ${kid} via HTTPS: ${url}"
    ...
    if [[ "${got}" -eq 0 ]]; then
      for ks in "${KEYSERVERS[@]}"; do
        log "Fetching Python OpenPGP key ${kid} from keyserver: ${ks}"
    ...
    [[ "${got}" -eq 1 ]] || die "Failed to fetch OpenPGP key ${kid} (HTTPS + all keyservers failed)"
  done
}
...
build_install_python() {
  ...
  # Keep configure conservative for compatibility; no LTO/PGO here.
  ./configure --prefix="${prefix}" --with-ensurepip=upgrade
  make -j "$(nproc)"

  # altinstall avoids clobbering "python3" system-wide
  as_root make altinstall

  log "Sanity check Python ${ver}"
  "${pybin}" -V
  "${pybin}" -c "import sys; print('exe=',sys.executable)"
  "${pybin}" -c "import ssl; print('ssl=',ssl.OPENSSL_VERSION)"
  "${pybin}" -c "import sqlite3,lzma,bz2,readline,uuid,ctypes,tkinter,hashlib; print('stdlib-ok')"
...
mkshim() {
  local tag="$1" pybin="$2"
  local version_suffix="${pybin##*python}" # e.g. "3.13"
...
exec \"${pybin}\" -m pip \"\$@\""
  ...
  printf "%s\n" "${wrap_py}"  > "${d}/python"
  printf "%s\n" "${wrap_py}"  > "${d}/python3"
  printf "%s\n" "${wrap_py}"  > "${d}/python${version_suffix}"

  printf "%s\n" "${wrap_pip}" > "${d}/pip"
  printf "%s\n" "${wrap_pip}" > "${d}/pip3"
  printf "%s\n" "${wrap_pip}" > "${d}/pip${version_suffix}"
...
install_miniforge() {
  log "Installing/Updating Miniforge (no conda init, no base auto-activate)"

  local installer="${BASE_DIR}/src/${MINIFORGE_FN}"
...
tests() {
  log "Testing python entrypoints"
...
main() {
  ...
  log "DONE."
  log "If you want it in current shell now:"
